...
if(empty($image)){   
    // get saved image
    $sql_img=$connect->query("SELECT * FROM products WHERE id='$id'");
    $img_data=$sql_img -> fetch_assoc();
    $img = $img_data['image'];

    // update data without changing image
    $update = $connect -> query("UPDATE products SET name='$name',price='$price',sale='$sale',seller='$seller',description='$description',image='$img' WHERE id='$id'");
